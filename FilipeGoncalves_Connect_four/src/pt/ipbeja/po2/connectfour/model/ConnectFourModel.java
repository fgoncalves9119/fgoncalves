...
public class ConnectFourModel {
    ...
    public ConnectFourModel(View view) {
        this.VIEW = view;
        this.dataBoard = new Cell[ROW_SIZE][COL_SIZE];
        this.fillBoard();
    }
    ...
    public void rollDown(int row, int col) {
        ...
        if (this.inWinnableTurn(turnCounter)) {
    ...
    public void updateBoardState(int row, int col) {
        ...
        if (this.turnCounter % 2 == 0) {
            ...
            dataBoard[row][col] = Cell.PLAYER2;
    ...
    public boolean hasWinningLine() {
        ...
        for (int line = ROW_SIZE - 1; line > 0; line--) {
            fourCounter = 1;
    ...
    public boolean hasWinningDiagonal() {
        for (int i = 0; i < ROW_SIZE; i++) {
            for (int j = 0; j < COL_SIZE; j++) {
                if (diagNeighboor(i, j) == 4 || antiDiagNeighboor(i, j) == 4) {
                    this.setScoreNumber();
                    VIEW.playerWins();
                    return true;
                }
    ...
    private int diagNeighboor(int row, int col) {
        int fourCounter = 1;
        while ((row - 1 >= 0 && col + 1 <= COL_SIZE - 1)) {
            if (this.dataBoard[row][col] == this.dataBoard[row - 1][col + 1] && this.dataBoard[row][col] != Cell.EMPTY) {
                fourCounter++;
                if (fourCounter == 4)
                    return fourCounter;
    ...
    public void redo() {
        if (movesRedo.size() > 0) {
            ...
            dataBoard[row][col] = plays.getCell();
    ...
    public boolean winningCondition(int col) {
        return hasWinningColumn(col) || hasWinningLine() || hasWinningDiagonal();
    }
    ...
    public boolean isDraw(int col) {
        if (turnCounter == 42 && !winningCondition(col)) {
            return true;
        }
        return false;
    }
    ...
    public boolean isFree(int row, int col) {
        return dataBoard[row][col] == Cell.EMPTY;
    }
    ...
    private void setScoreNumber() {
        this.score = (ROW_SIZE * COL_SIZE) - this.turnCounter;
    }
