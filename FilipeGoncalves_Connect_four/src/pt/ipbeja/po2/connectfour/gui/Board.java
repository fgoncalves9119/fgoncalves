...
public class Board extends VBox implements View {
    ...
    /**
     * Sort the highscores and add the information to the alert that will show the scores
     */
    @Override
    ...
    public void redo(int row, int col) {
        CellButton btn = this.buttons[row][col];
        Cell cell = this.gameModel.getMovesRedo().get(gameModel.getMovesRedo().size() - 1).getCell();
